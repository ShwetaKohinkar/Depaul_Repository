...
final class InventorySet implements Inventory {
    ...
    void replaceEntry(Video video, Record record) {
       ...
       Video vid = checkRecord(video).video();
        _data.remove(vid);
        if (record != null)
            _data.put(vid, ((RecordObj) record).copy());
    }
    ...
    Record checkIn(Video video) {
        ...
        _data.replace(r.video(), new RecordObj(r.video(), r.numOwned(), r.numOut() - 1, r.numRentals()));
        return r;
    }
