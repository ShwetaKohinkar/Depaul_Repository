...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    if xs == Nil then 
      ...
      return true
    else if xs.head != xs.tail.head then 
      return false
    else 
      allEqual(xs.tail)
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ...
    if ys==Nil then 
      Nil
    else if ys.head==x then 
      delete1(ys.head,ys.tail)
    else
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    def temp [X] (x:X, xs:List[X],k:Int) :List[(Int,X)] = {
      if xs == Nil then
        if x == Nil then List()
        else (k,x) :: List()
      else
        if xs.head == x then
          ...
          (k,x) :: temp(xs.head,xs.tail,1)
    }
      if xs.length > 0 then 
        temp(xs.head,xs.tail,1)
      else 
        List()
    ...
    //             else (k,x)::List()
    //  case y::yt => if y == x then temp(x,yt,k+1) 
    //                else (k,x) :: temp(y,yt,1)}
    //               }
    //               if xs.length > 0 then temp(xs.head,xs.tail,1) 
    //               else List()
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    else if n == -1 && xs.tail == Nil then (List(), xs)
    else 
      val(a,b) = splitAt(n-1,xs.tail) 
      if n > 0 then 
        (xs.head :: a,b)
      else 
        (a,xs.head :: b)
  ...
  def allDistinct(xs: List[Int]): Boolean =
    ...
    if xs == Nil then 
      ...
      if member(xs.head, xs.tail) then 
       return false
      else allDistinct(xs.tail)
