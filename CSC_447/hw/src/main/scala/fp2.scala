...
object fp2:
  ...
  def joinTerminateRight(xs: List[String], term: String): String ={
   def f(splitt : String, x: String) : String = {term + splitt + x}
    if xs == Nil then "" 
      ...
      xs.head + foldRight(xs.tail,term,f)
  }
  ...
  def firstNumGreaterThan(a: Int, xs: List[Int]): Int ={
   ...
   else if xs.head < a then 
     firstNumGreaterThan(a,xs.tail) 
   else 
     xs.head
  }
  ...
  def firstIndexNumGreaterThan(a: Int, xs: List[Int]): Int ={
    xs.indexOf(firstIndexNumAux(a,xs))
  }

  def firstIndexNumAux(a:Int,xs:List[Int]): Int ={
    if xs == Nil then
       throw new RuntimeException
    else if xs.head < a then 
      firstIndexNumAux(a,xs.tail)
    else 
      xs.head
